...
`run_all.m` chains every step (data → EDA → diagnostic → estimation → counterfactual → decomposition) and produces every figure and table cited in `report/main.tex`. End-to-end wall time: ~15–35 min depending on the machine. Optional flags at the top of `run_all.m` (`SKIP_ESTIMATION`, `SKIP_DBNOMICS`, …) let you skip the slow steps and re-use the cached posterior.
...
| 4 | `run_estimation_UEM.m`             | `build/estimation_UEM/Output/*_results.mat`, posterior figures, `data/tables/posterior_summary.csv`, `data/tables/variance_decomposition.csv` | 10–30 min |
...
- **Inference at the mode.** MCMC chains under-mix because the numerical Hessian is non-PD; we use `MCMC_jumping_covariance = 'prior_variance'` and rely on the Laplace approximation around the mode (Herbst–Schorfheide 2015 §6) for posterior moments. Documented honestly in §5.2 of the report.
